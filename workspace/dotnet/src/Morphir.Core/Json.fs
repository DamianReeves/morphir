...
module Encode =
  ...
  let list (func: 'a -> Value) (values: #seq<'a>) : Value =
    ...
    values |> Seq.fold addEntry (JArray()) :> Value
...
module Decode =
  ...
  let inline andThen (cb: 'a -> Decoder<'b>) (decoder: Decoder<'a>) : Decoder<'b> = Decode.andThen cb decoder
  ...
  let inline field (fieldName: string) (decoder: Decoder<'a>) : Decoder<'a> = Decode.field fieldName decoder

  let inline index (requestedIndex: int) (decoder: Decoder<'a>) = Decode.index requestedIndex decoder
  ...
  let inline map2 (ctor: 'a -> 'b -> 'c) (decoder1: Decoder<'a>) (decoder2: Decoder<'b>) : Decoder<'c> =
