...
let rec encoder (encodeAttributes: 'a -> Value) (tpe: Type<'a>) : Encode.Value =
  ...
  | Type.Variable(attr, name) -> Encode.list id [ Encode.string "Variable"; encodeAttributes attr; Name.encoder name ]
...
let rec decoder (decodeAttributes: Decode.Decoder<'a>) : Decode.Decoder<Type<'a>> =
  ...
  |> Decode.andThen (function
    | "Variable" -> Decode.map2 Type.variable (Decode.index 1 decodeAttributes) (Decode.index 2 Name.decoder)
    | "Reference" ->
      Decode.map3
        Type.reference
    ...
    | "Tuple" ->
      Decode.map2 Type.tuple (Decode.index 1 decodeAttributes) (Decode.index 2 (Decode.list (decoder decodeAttributes)))
    | "Record" ->
      Decode.map2
        Type.record
        (Decode.index 1 decodeAttributes)
...
let decodeField decodeAttributes : Decode.Decoder<Field<'a>> =
  Decode.map2 field (Decode.field "name" Name.Codec.decodeName) (Decode.field "tpe" (decoder decodeAttributes))
...
let decodeConstructors decodeAttributes : Decode.Decoder<Constructors<'a>> =
  Decode.list (
    Decode.map2
      ...
      (Decode.index
        1
        (Decode.list (Decode.map2 Tuple.pair (Decode.index 0 Name.decoder) (Decode.index 1 (decoder decodeAttributes)))))
  )
...
let encodeSpecification encodeAttributes spec : Value =
  ...
  | OpaqueTypeSpecification typeParams ->
    Encode.list id [ Encode.string "OpaqueTypeSpecification"; Encode.list Name.encoder typeParams ]
  ...
  |> Decode.andThen (function
    | "TypeAliasSpecification" ->
      Decode.map2
        ...
        (Decode.index 2 (decoder decodeAttributes))
    | "OpaqueTypeSpecification" -> Decode.map opaqueTypeSpecification (Decode.index 1 (Decode.list Name.decoder))
