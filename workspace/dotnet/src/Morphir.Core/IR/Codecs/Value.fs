    ...
    Encode.list
      ...
      [ Encode.string "record"
        ...
        |> Encode.list (fun (name, value) ->
          Encode.list id [ Name.encoder name; encoder encodeTypeAttributes encodeValueAttributes value ]) ]
...
let decoder
  ...
  |> Decode.andThen (fun kind ->
    match kind with
    | "literal" -> Decode.map2 literal (Decode.index 1 decodeValueAttributes) (Decode.index 2 Literal.decoder)
    | "constructor" -> Decode.map2 constructor (Decode.index 1 decodeValueAttributes) (Decode.index 2 FQName.decoder)
    | "tuple" ->
      Decode.map2
        tuple
    ...
    | "variable" -> Decode.map2 variable (Decode.index 1 decodeValueAttributes) (Decode.index 2 Name.decoder)
    | "reference" -> Decode.map2 reference (Decode.index 1 decodeValueAttributes) (Decode.index 2 FQName.decoder)
    | "field" ->
      Decode.map3
        field
        (Decode.index 1 decodeValueAttributes)
    ...
    | "field_function" ->
      Decode.map2 fieldFunction (Decode.index 1 decodeValueAttributes) (Decode.index 2 Name.decoder)
    | "apply" ->
      Decode.map3
        apply
        (Decode.index 1 decodeValueAttributes)
      ...
      Decode.map3
        ...
        (Decode.index
          ...
          (Decode.list (
           ...
           |> Decode.map Dict.fromList))
    | "unit" -> Decode.map unit (Decode.index 1 decodeValueAttributes)
    | other -> Decode.fail ($"Unknown pattern value: {other}"))

let rec encodePattern (encodeAttributes: 'va -> Value) (pattern: Pattern<'va>) : Value =
  match pattern with
  | WildcardPattern attributes -> Encode.list id [ Encode.string "wildcard_pattern"; encodeAttributes attributes ]
  ...
  | EmptyListPattern attributes -> Encode.list id [ Encode.string "empty_list_pattern"; encodeAttributes attributes ]
  ...
  | LiteralPattern(attributes, value) ->
    Encode.list
      ...
      [ Encode.string "literal_pattern"
        encodeAttributes attributes
        Literal.encoder value ]
  | UnitPattern attributes -> Encode.list id [ Encode.string "unit_pattern"; encodeAttributes attributes ]

let rec decodePattern (decodeAttributes: Decode.Decoder<'va>) : Decode.Decoder<Pattern<'va>> =
  Decode.index 0 Decode.string
  |> Decode.andThen (fun kind ->
    match kind with
    | "wildcard_pattern" -> Decode.map wildcardPattern (Decode.index 1 decodeAttributes)
    | "as_pattern" ->
      Decode.map3
        asPattern
    ...
    | "empty_list_pattern" -> Decode.map emptyListPattern (Decode.index 1 decodeAttributes)
    | "head_tail_pattern" ->
      Decode.map3
        headTailPattern
        (Decode.index 1 decodeAttributes)
    ...
    | "literal_pattern" ->
      Decode.map2 literalPattern (Decode.index 1 decodeAttributes) (Decode.index 2 Literal.decoder)
    | "unit_pattern" -> Decode.map unitPattern (Decode.index 1 decodeAttributes)
  ...
  Encode.object
    [ "inputs",
      ...
      |> Encode.list (fun (argName, argType) ->
        Encode.list id [ Name.encoder argName; Type.encoder encodeAttributes argType ])
...
let decodeSpecification decodeAttributes : Decode.Decoder<Specification<'a>> =
  Decode.map2
    ...
    (Decode.field
      "inputs"
      (Decode.list (
        Decode.map2 Tuple.pair (Decode.index 0 Name.decoder) (Decode.index 1 (Type.decoder decodeAttributes))
      )))
