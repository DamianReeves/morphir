...
module Name =
  ...
  let decoder: Decoder<Name> = Decode.list Decode.string |> Decode.map Name.fromList

[<RequireQualifiedAccess>]
module Path =
  let inline encoder (path: Path) : Value = Encode.list Name.encoder path

  let decoder: Decoder<Path> = Decode.list Name.decoder |> Decode.map fromList
...
module FQName =
  let encoder =
    ...
    | FQName(packagePath, modulePath, localName) ->
      Encode.list id [ Path.encoder packagePath; Path.encoder modulePath; Name.encoder localName ]

  let decoder: Decoder<FQName> =
    Decode.map3 fQName (Decode.index 0 Path.decoder) (Decode.index 1 Path.decoder) (Decode.index 2 Name.decoder)
