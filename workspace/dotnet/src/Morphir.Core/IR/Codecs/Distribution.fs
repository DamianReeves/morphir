      ...
      [ Encode.string "library"
        ...
        (dependencies
         ...
         |> Encode.list (fun (packageName, packageSpec) ->
           Encode.list id [ Path.encoder packageName; Package.encodeSpecification encodeUnit packageSpec ]))
    ...
    [ Decode.field "formatVersion" Decode.int
      |> Decode.andThen (fun formatVersion ->
        if formatVersion = currentFormatVersion then
          ...
          Decode.fail
            $"The IR is using format (version {formatVersion}) but the latest format version is {currentFormatVersion}. Please regenerate it!")
