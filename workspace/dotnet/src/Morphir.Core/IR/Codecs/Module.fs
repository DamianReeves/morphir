...
let encodeSpecification encodeTypeAttributes (spec: Specification<'ta>) : Value =
  Encode.object
    [ "types",
      ...
      "doc", spec.Doc |> Maybe.map Encode.string |> Maybe.withDefault Encode.nil ]
    ...
    [ "types",
      ...
      |> Encode.list (fun (name, typeDef) ->
        Encode.list
          ...
          [ Name.encoder name
            typeDef
            |> (AccessControlled.encoder (Documented.encoder (Type.encodeDefinition encodeTypeAttributes))) ])
      "values",
      def.Values
      |> Dict.toList
      |> Encode.list (fun (name, valueDef) ->
        Encode.list
          ...
          [ Name.encoder name
            valueDef
            |> (AccessControlled.encoder (
              Documented.encoder (Value.encodeDefinition encodeTypeAttributes encodeValueAttributes)
            )) ])
      "def", def.Doc |> Maybe.map Encode.string |> Maybe.withDefault Encode.nil ]

let decodeDefinition
      ...
      (Decode.map
        ...
        (Decode.list (
          Decode.map2
            ...
            (Decode.field
              "def"
              (AccessControlled.decoder (Documented.decoder (Type.decodeDefinition decodeTypeAttributes))))
        ))))
    (Decode.field
      "values"
      (Decode.map
        ...
        (Decode.list (
          Decode.map2
            ...
            (Decode.index
              1
              (AccessControlled.decoder (
                Documented.decoder (Value.decodeDefinition decodeTypeAttributes decodeValueAttributes)
              )))
        ))))
    (Decode.oneOf
