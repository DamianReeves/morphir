...
let lookupTypeSpecification (localName: Name) (moduleSpec: Specification<'ta>) : Maybe<Type.Specification<'ta>> =
  moduleSpec.Types |> Dict.get localName |> Maybe.map Documented.value

let lookupValueSpecification (localName: Name) (moduleSpec: Specification<'ta>) : Maybe<Value.Specification<'ta>> =
  moduleSpec.Values |> Dict.get localName |> Maybe.map Documented.value

/// Look up a value definition by its name in a module specification.
let lookupValueDefinition localName (moduleDef: Definition<'ta, 'va>) : Maybe<Value.Definition<'ta, 'va>> =
  moduleDef.Values
  |> Dict.get localName
  |> Maybe.map (withPrivateAccess >> Documented.value)
...
let definitionToSpecification (def: Definition<'TA, 'VA>) : Specification<'TA> =
  { Types =
      ...
      |> List.filterMap (fun (path, accessControlledType) ->
        ...
        |> withPublicAccess
        |> Maybe.map (fun typeDef -> (path, typeDef |> Documented.map Type.definitionToSpecification)))
      |> Dict.fromList
    Values =
      def.Values
      |> Dict.toList
      |> List.filterMap (fun (path, accessControlledValue) ->
        ...
        |> withPublicAccess
        |> Maybe.map (fun valueDef -> (path, valueDef |> Documented.map Value.definitionToSpecification)))
      |> Dict.fromList
    Doc = def.Doc }
